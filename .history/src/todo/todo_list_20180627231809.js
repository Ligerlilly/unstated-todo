...
class TodoListContainer extends Container {
    ...
    toggleFinished(todo) {
        const todoIndex = this.state.todos.indexOf(t => t.id = todo.id)
        todo.finished = !todo.finished
        this.setState([...this.state.todos.slice(0, todoIndex), todo, ...this.state.todos.slice(todoIndex + 1, this.state.todos.length)])
    }
}
...
class TodoList extends React.Component {
    ...
    addTodo(todoListContainer) {
        todoListContainer.addTodo({...this.state.todo, id: Math.random()})
    ...
    render() {
        return (
            <Subscribe to={[TodoListContainer]}>
                {todoList => (
                    <div>
                        <ul style={{ listStyle: "none" }}>{todoList.state.todos.map((todo, i) => <TodoView key={i} todo={todo} toggleFinished={()=> todoList.toggleFinished(todo)} /> )}</ul>
